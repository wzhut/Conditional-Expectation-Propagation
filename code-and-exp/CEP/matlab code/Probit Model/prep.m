function [logl, KL, auc, logls, KLs, aucs, time] = prep(train, test, ts_mean, ts_var, cfg)
...
while true
    ...
    for d = 1 : D
%         idx = setdiff(1:D, d);
%         num = (2*y - 1) .* sum(x .* q_mean, 2);
%         den = sqrt( 1 + sum(x.^2 .* q_var,2));
%         cdf = normcdf(num ./ den)+realmin;
%         pdf = normpdf(num ./ den)+realmin;
%         dmu = pdf ./ cdf .* (2 * y - 1) .* x(:,d) ./ den;
%         dsig = -0.5 * pdf ./ cdf .* num ./ (den.^3) .*  (x(:,d).^2);
        c1 = (2 * y - 1) ./ sqrt( 1 + sum(x.^2 .* q_var,2));
        c2 = sum(x .* q_mean, 2);
...
%         new_mean(tmp, d) = mean_list(tmp ,d);
    ...
    disp(sprintf('ep-- iter:%d, diff:%.8f, train_logl:%.8f, test_logl:%.8f, auc:%.8f,  KL:%.8f',iter, diff, train_logl, logl ,auc, KL)); 
    iter = iter + 1;
    if diff < threshold || iter > max_iter %|| abs(last_train_logl - train_logl) < threshold * 1e-3
        save('./intermediate/prep.mat', 'intermediate_record');
        break;
    end
