function [logl, KL, auc, logls, KLs, aucs, time] = prep_nf(train,test, ts_mean, ts_var, cfg)
...
while true
    ...
    disp(sprintf('prep_nf -- iter:%d, diff:%.8f, train_logl:%.8f, test_logl:%.8f, auc:%.8f,  KL:%.8f',iter, diff, train_logl, logl.mean ,auc, KL));
    if diff < threshold || iter > max_iter 
        save('./intermediate/prcep_nf.mat', 'intermediate_record');
        break;
    end
    iter = iter + 1;
end
