function [logl, KL, auc, logls, KLs, aucs, time] = prepis(train, test, ts_mean, ts_var)
...
while true
    ...
    disp(sprintf('prepis-- iter:%d, diff:%.8f, train_logl:%.8f, test_logl:%.8f, auc:%.8f,  KL:%.8f',iter, diff, train_logl, logl.mean ,auc, KL)); 
    iter = iter + 1;
