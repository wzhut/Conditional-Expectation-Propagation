function [logl, KL, auc, logls, KLs, aucs, time] = lrlp(train,test, ts_mean, ts_var, cfg)
...
%         new_mean(tmp) = mean_list(tmp);
    ...
    disp(sprintf('lrlp -- iter:%d, diff:%.8f, train_logl:%.8f, test_logl:%.8f, auc:%.8f,  KL:%.8f',iter, diff, train_logl, logl ,auc, KL));
