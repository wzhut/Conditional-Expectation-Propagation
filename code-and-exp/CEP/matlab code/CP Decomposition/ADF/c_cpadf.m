function [mse, rmse, iter, diffs, joint] = c_cpadf(train, test, rank, dim, batch_size, cfg)
...
for nbatch = 1 : num_batch
    ...
    for m = 1: 3
        for i = 1 : dim(m)
            ...
            joint_precision = num * eye(rank) * 1e-6 + joint{m}.precision(:,:,i);
...
%         diffs(iter) = diff;
        ...
        if cfg.verbose == 1
            ...
            train_error = mean((y_list - sum(predicted, 1)').^2);
            % test error
            M = size(test,1);
            ...
            disp(sprintf('batch:%d, iter:%d -- diff:%.8f train_error:%.8f test_error:%.8f',nbatch, iter,diff,train_error,test_error));
        end
