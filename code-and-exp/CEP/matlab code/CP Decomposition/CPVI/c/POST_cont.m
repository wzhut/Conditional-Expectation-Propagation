...
function [post_u, iter, test_rmses, time] = POST_cont(prior, batch, opt, test)
    prior_mean = prior{1}; 
    prior_cov = prior{2};
    prior_a = prior{3}(1);
    prior_b = prior{3}(2);
    ...
    train_rmses = zeros([opt.max_iter, 1]);
    test_rmses = zeros([opt.max_iter, 1]);
    time = zeros([opt.max_iter, 1]);
    ...
    for iter = 1:opt.max_iter
        ...
        pred = sum(mu,2);
        err = mean((true_val - pred).^2);    
        test_rmses(iter) = sqrt(err);
        fprintf('iter = %d, diff = %g\n', iter, diff);
        if diff< opt.tol
            break;
        end
        
        if iter > 10 && mean(abs(train_rmses(iter-9: iter-1) - train_rmses(iter))) < opt.tol
            break;
        end
        
    end
    post_u = {post_mean, post_cov, [a,b]};
end