...
function [U_invS_g, U_invSMean_g] = b_cpcep_zhe( train, test, dim, nvec, cfg)
    [N, nmod] = size(train);
    nmod = nmod - 1;
    ...
    for iter = 1:cfg.max_iter
        ...
        for k=1:nmod
            ...
            if k==1
                s_mean_not = reshape(mtimesx(U_mean_not{k},'t', z_nk),[N,1]); 
                s_var_not = sum(reshape(z_nk2.*(U_cov_not{k} + mtimesx(U_mean_not{k},U_mean_not{k}, 't')), ...
                    [dim*dim, N]),1)';
